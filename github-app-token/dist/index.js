...
async function run() {
    try {
        ...
        if (!owner) {
            throw new Error("Owner was not supplied as an input or environment variable");
        }
        if (!repository) {
        ...
        const { data: installation } = await octokit.request("GET /repos/{owner}/{repository}/installation", {
            owner,
            repository
        });
